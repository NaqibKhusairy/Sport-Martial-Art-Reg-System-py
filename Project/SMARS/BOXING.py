...
def bx():																					#bx class
	...
	def bxngsbmt():																			#function bxngsbmt
		...
		try:																				#try to do
			...
			category=ctgory.Bxg(Weight)														#Set category and call Bxg function from ctgory
		...
		def cncl():																			#cncl function
			try:																			#try to do
				...
				mydbse.execute("DELETE FROM boxing WHERE nric=%s",							#Executing database query with parameters
					(Ic,))
		...
		try:																				#try to do
			...
			mydbse = projectdatabase.cursor()												#Instantiating a cursor for myprojectdb
			mydbse.execute("SELECT * FROM boxing WHERE nric=%s",							#Executing database query with parameters
				(Ic,))
			...
			if sameinpt:																	#if sameinpt, do
				...
				mydbse.execute("INSERT INTO boxing"											#insert data into customers table
	...
	wnds.title("Boxing Registration System")												#Set title
	wnds.configure(bg="#faf8f5")															#Set Background color

	wlcm=prjk.Label(wnds,text="\n  ***Welcome To Boxing Registration System***\n",			#Create Label
	...
	submitBtn=prjk.Button(BtnFrm,text="SUBMIT",command=bxngsbmt)							#Create Submit button
	submitBtn.grid(row=0,column=0,padx=5)													#set position with grid and padding

	rstBtn=prjk.Button(BtnFrm,text="RESET",command=bxngrst)									#Create Reset button
