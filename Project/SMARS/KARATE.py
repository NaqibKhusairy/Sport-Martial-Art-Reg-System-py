...
def krt():																					#krt function
	def krtrst():																			#function krtrst
	...
	def krtsbmt():																			#function krtsbmt
		...
		try:																				#try to do
			...
			category=ctgory.Krt(Weight)														#Set category and call Krt function from ctgory
		...
		def cncl():																			#cncl function
			try:																			#try to do
				...
				mydbse.execute("DELETE FROM karate WHERE nric=%s",							#Executing database query with parameters
					(Ic,))
		...
		confwnds.geometry("375x420")														#Set winow size = 400 X 650
		...
		nLbl.grid(row=2,column=0,padx=5,pady=5)												#set position with grid and padding
		nDot=prjk.Label(DataFrame,text=" : ", bg="#faf8f5")									#Create Label for :
		nDot.grid(row=2,column=1,padx=5,pady=5)												#set position with grid and padding
		n=prjk.Entry(DataFrame)																#Create n Entry
		n.insert(prjk.END,str(Ic))															#insert string Ic to n Entry
		n.config(state='disable')															#make n Entrty disable
		n.grid(row=2,column=2,padx=5,pady=5)												#set position with grid and padding

		eLbl=prjk.Label(DataFrame,text="E-MAIL", bg="#faf8f5")								#Create Label for E-MAIL
		eLbl.grid(row=3,column=0,padx=5,pady=5)												#set position with grid and padding
		eDot=prjk.Label(DataFrame,text=" : ", bg="#faf8f5")									#Create Label for :
		eDot.grid(row=3,column=1,padx=5,pady=5)												#set position with grid and padding
		e=prjk.Entry(DataFrame)																#Create e Entry
		e.insert(prjk.END,Mail)																#insert Mail to e Entry
		e.config(state='disable')															#make e Entrty disable
		e.grid(row=3,column=2,padx=5,pady=5)												#set position with grid and padding

		pLbl=prjk.Label(DataFrame,text="PHONE NUMBER", bg="#faf8f5")						#Create Label for PHONE NUMBER
		pLbl.grid(row=4,column=0,padx=5,pady=5)												#set position with grid and padding
		pDot=prjk.Label(DataFrame,text=" : ", bg="#faf8f5")									#Create Label for :
		pDot.grid(row=4,column=1,padx=5,pady=5)												#set position with grid and padding
		p=prjk.Entry(DataFrame)																#Create p Entry
		p.insert(prjk.END,PhoneNum)															#insert PhoneNum to p Entry
		p.config(state='disable')															#make p Entrty disable
		p.grid(row=4,column=2,padx=5,pady=5)												#set position with grid and padding

		wLbl=prjk.Label(DataFrame,text="WEIGHT", bg="#faf8f5")								#Create Label for WEIGHT
		wLbl.grid(row=5,column=0,padx=5,pady=5)												#set position with grid and padding
		wDot=prjk.Label(DataFrame,text=" : ", bg="#faf8f5")									#Create Label for :
		wDot.grid(row=5,column=1,padx=5,pady=5)												#set position with grid and padding
		w=prjk.Entry(DataFrame)																#Create w Entry
		w.insert(prjk.END,Weight)															#insert Weight to w Entry
		w.config(state='disable')															#make w Entrty disable
		w.grid(row=5,column=2,padx=5,pady=5)												#set position with grid and padding

		cLbl=prjk.Label(DataFrame,text="CATEGORY", bg="#faf8f5")							#Create Label for CATEGORY
		cLbl.grid(row=6,column=0,padx=5,pady=5)												#set position with grid and padding
		cDot=prjk.Label(DataFrame,text=" : ", bg="#faf8f5")									#Create Label for :
		cDot.grid(row=6,column=1,padx=5,pady=5)												#set position with grid and padding
		c=prjk.Entry(DataFrame)																#Create c Entry
		c.insert(prjk.END,category)															#insert category to c Entry
		c.config(state='disable')															#make c Entrty disable
		c.grid(row=6,column=2,padx=5,pady=5)												#set position with grid and padding
		...
		try:																				#try to do
			...
			mydbse = projectdatabase.cursor()												#Instantiating a cursor for myprojectdb
			mydbse.execute("SELECT * FROM karate WHERE nric=%s",							#Executing database query with parameters
				(Ic,))
			...
			if sameinpt:																	#if sameinpt, do
				...
				mydbse.execute("INSERT INTO karate"											#insert data into customers table
	...
	wnds.title("Karate Registration System")												#Set title
	wnds.configure(bg="#faf8f5")															#Set Background color

	wlcm=prjk.Label(wnds,text="\n  ***Welcome To Karate Registration System***\n",			#Create Label
	...
	submitBtn=prjk.Button(BtnFrm,text="SUBMIT",command=krtsbmt)								#Create Submit button
	submitBtn.grid(row=0,column=0,padx=5)													#set position with grid and padding

	rstBtn=prjk.Button(BtnFrm,text="RESET",command=krtrst)									#Create Reset button
