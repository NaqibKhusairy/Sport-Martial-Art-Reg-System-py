...
def mma():																					#mma function
	...
	def mmasbmt():																			#function mmasbmt
		...
		try:																				#try to do
			...
			category=ctgory.MmA(Weight)														#Set category and call MmA function from ctgory
		...
		def cncl():																			#cncl function
			try:																			#try to do
				...
				mydbse.execute("DELETE FROM mma WHERE nric=%s",								#Executing database query with parameters
					(Ic,))
		...
		try:																				#try to do
			...
			mydbse = projectdatabase.cursor()												#Instantiating a cursor for myprojectdb
			mydbse.execute("SELECT * FROM mma WHERE nric=%s",								#Executing database query with parameters
				(Ic,))
			...
			if sameinpt:																	#if sameinpt, do
				...
				mydbse.execute("INSERT INTO mma"											#insert data into customers table
	...
	wnds.title("MMA Registration System")													#Set title
	wnds.configure(bg="#faf8f5")															#Set Background color

	wlcm=prjk.Label(wnds,text="\n  ***Welcome To MMA Registration System***\n",				#Create Label
	...
	submitBtn=prjk.Button(BtnFrm,text="SUBMIT",command=mmasbmt)								#Create Submit button
	submitBtn.grid(row=0,column=0,padx=5)													#set position with grid and padding

	rstBtn=prjk.Button(BtnFrm,text="RESET",command=mmarst)									#Create Reset button
