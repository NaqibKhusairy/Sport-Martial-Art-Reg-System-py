...
def mt():																					#mt function
	...
	def mytosbmt():																			#function mytosbmt
		...
		try:																				#try to do
			...
			category=ctgory.Mythai(Weight)													#Set category and call Mythai function from ctgory
		...
		def cncl():																			#cncl function
			try:																			#try to do
				...
				mydbse.execute("DELETE FROM muaythai WHERE nric=%s",						#Executing database query with parameters
					(Ic,))
		...
		SMA="Muay Thai"																		#Set SMA = Muay Thai
		try:																				#try to do
			projectdatabase = mysql.connector.connect(										#connect to database
				host="localhost", 
				user="root", 
			...
			mydbse = projectdatabase.cursor()												#Instantiating a cursor for myprojectdb
			mydbse.execute("SELECT * FROM muaythai WHERE nric=%s",							#Executing database query with parameters
				(Ic,))
			...
			if sameinpt:																	#if sameinpt, do
				...
				mydbse.execute("INSERT INTO muaythai"											#insert data into customers table
	...
	wnds.title("Muay Thai Registration System")												#Set title
	wnds.configure(bg="#faf8f5")															#Set Background color

	wlcm=prjk.Label(wnds,text="\n  ***Welcome To Muay Thai Registration System***\n",		#Create Label
	...
	submitBtn=prjk.Button(BtnFrm,text="SUBMIT",command=mytosbmt)							#Create Submit button
	submitBtn.grid(row=0,column=0,padx=5)													#set position with grid and padding

	rstBtn=prjk.Button(BtnFrm,text="RESET",command=mytorst)									#Create Reset button
