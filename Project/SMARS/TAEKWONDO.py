...
def taekwondo():																			#taekwondo function
	def tndorst():																			#function tndorst
	...
	def tndosbmt():																			#function tndosbmt
		...
		try:																				#try to do
			...
			year=int(Ic[0:2])																#get year from Ic parameter 0-1
			...
			category=ctgory.Twdo(Weight)													#Set category and call Twdo function from ctgory
		...
		def cncl():																			#cncl function
			try:																			#try to do
				...
				mydbse.execute("DELETE FROM taekwondo WHERE nric=%s",						#Executing database query with parameters
					(Ic,))
		...
		SMA="Taekwondo"																		#Set SMA = Muay Thai
		try:																				#try to do
			projectdatabase = mysql.connector.connect(										#connect to database
				host="localhost", 
				user="root", 
			...
			mydbse = projectdatabase.cursor()												#Instantiating a cursor for myprojectdb
			mydbse.execute("SELECT * FROM taekwondo WHERE nric=%s",							#Executing database query with parameters
				(Ic,))
			...
			if sameinpt:																	#if sameinpt, do
				...
				mydbse.execute("INSERT INTO taekwondo"										#insert data into customers table
	...
	wnds.title("Taekwondo Registration System")												#Set title
	wnds.configure(bg="#faf8f5")															#Set Background color

	wlcm=prjk.Label(wnds,text="\n  ***Welcome To Taekwondo Registration System***\n",		#Create Label
	...
	submitBtn=prjk.Button(BtnFrm,text="SUBMIT",command=tndosbmt)							#Create Submit button
	submitBtn.grid(row=0,column=0,padx=5)													#set position with grid and padding

	rstBtn=prjk.Button(BtnFrm,text="RESET",command=tndorst)									#Create Reset button
